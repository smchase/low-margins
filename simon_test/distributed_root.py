import sys
import time
from pathlib import Path
import cv2
import torch
import torch.optim as optim
from torchvision import datasets, transforms
from torch.utils.data import Subset, DataLoader

sys.path.insert(0, str(Path(__file__).parent.parent))
from camera import Camera, Frame, SECONDS_PER_FRAME, RECEIVE_OFFSET  # noqa: E402
from train.model import MLP  # noqa: E402
from train.worker import Worker  # noqa: E402
from simon_test.distributed_utils import (  # noqa: E402
    DistributedState, get_current_state_from_time, wait_for_state,
    tensors_to_frames, frames_to_tensors, get_parameter_shapes,
    log_timing, CYCLE_DURATION, ROOT_SEED
)


def transmit_frames_properly(cam: Camera, frames: list[Frame], phase_start_time: float):
    """Transmit frames ensuring each is displayed properly."""
    for i, frame in enumerate(frames):
        # Transmit the frame
        cam.transmit(frame)
        log_timing(f"  Transmitted frame {i+1}/{len(frames)}")
        
        # Keep displaying this frame until it's time for the next one
        next_frame_time = (i + 1) * SECONDS_PER_FRAME
        while (time.time() - phase_start_time) < next_frame_time:
            cam.update()  # This keeps the frame visible!
            cv2.waitKey(10)


def receive_frames_properly(cam: Camera, phase_start_time: float, num_frames: int) -> list[Frame]:
    """Receive frames with proper timing."""
    received_frames = []
    
    for i in range(num_frames):
        # Wait with configured offset for frame to be displayed
        target_time = i * SECONDS_PER_FRAME + RECEIVE_OFFSET
        while (time.time() - phase_start_time) < target_time:
            cam.update()
            cv2.waitKey(10)
        
        # Capture frame
        frame = cam.receive()
        received_frames.append(frame)
        log_timing(f"  Received frame {i+1}/{num_frames}")
    
    return received_frames


def wait_until(cam: Camera, target_time: float):
    """Wait until target time while keeping camera updated."""
    while time.time() < target_time:
        cam.update()
        cv2.waitKey(30)


def main():
    # Parse arguments
    import argparse
    parser = argparse.ArgumentParser(description='Distributed training root node')
    parser.add_argument('--num-steps', type=int, default=10, help='Number of training steps')
    parser.add_argument('--learning-rate', type=float, default=0.01, help='Learning rate')
    args = parser.parse_args()
    
    # Initialize camera
    cam = Camera()
    
    log_timing("=== DISTRIBUTED TRAINING - ROOT NODE ===")
    log_timing(f"Training for {args.num_steps} steps")
    log_timing(f"Cycle duration: {CYCLE_DURATION}s")
    
    if not cam.calibrate():
        log_timing("ERROR: Calibration failed")
        return
    
    log_timing("Calibration complete. Press SPACE to start...")
    
    while True:
        cam.update()
        if (cv2.waitKey(30) & 0xFF) == ord(' '):
            break
    
    # Set random seed
    torch.manual_seed(ROOT_SEED)
    
    # Create model
    device = 'cpu'
    model = MLP().bfloat16().to(device)
    optimizer = optim.Adam(model.parameters(), lr=args.learning_rate)
    param_shapes = get_parameter_shapes(model)
    
    # Load dataset
    transform = transforms.Compose([
        transforms.ToTensor(),
    ])
    
    train_dataset = datasets.MNIST(
        root='./data',
        train=True,
        download=True,
        transform=transform
    )
    
    # Root gets first half of data
    data_indices = list(range(len(train_dataset) // 2))
    data_subset = Subset(train_dataset, data_indices)
    
    worker = Worker(
        model=model,
        data_subset=data_subset,
        batch_size=1024,
        learning_rate=args.learning_rate,
        device=device,
        root=True,
        worker_id=0
    )
    
    # Load test dataset
    test_dataset = datasets.MNIST(
        root='./data',
        train=False,
        download=True,
        transform=transform
    )
    test_loader = DataLoader(test_dataset, batch_size=1000, shuffle=False)
    
    # Calculate frames needed
    test_params = [p.clone() for p in model.parameters()]
    param_frames = tensors_to_frames(test_params)
    num_param_frames = len(param_frames)
    log_timing(f"Model has {sum(p.numel() for p in model.parameters())} parameters")
    log_timing(f"Need {num_param_frames} frames for parameters/gradients")
    
    # Wait to start at beginning of COMPUTE_G
    log_timing("Waiting to synchronize to global clock...")
    wait_for_state(DistributedState.COMPUTE_G)
    
    start_time = time.time()
    log_timing(f"Starting training at unix time {start_time:.6f}")
    
    collected_gradients = []
    total_train_loss = 0.0
    
    for step in range(args.num_steps):
        cycle_start = time.time()
        
        # ========== State 1: COMPUTE_G ==========
        state_start = wait_for_state(DistributedState.COMPUTE_G)
        log_timing(f"Step {step+1}: Computing gradients", DistributedState.COMPUTE_G)
        
        # Compute local gradients
        local_gradients, local_loss = worker.run_step()
        total_train_loss += local_loss
        collected_gradients = [local_gradients]
        
        # ========== State 2: COMMUNICATE_G ==========
        state_start = wait_for_state(DistributedState.COMMUNICATE_G)
        log_timing(f"Step {step+1}: Receiving gradients from worker", DistributedState.COMMUNICATE_G)
        
        phase_start = time.time()
        received_frames = receive_frames_properly(cam, phase_start, num_param_frames)
        
        try:
            worker_gradients = frames_to_tensors(received_frames, param_shapes)
            collected_gradients.append(worker_gradients)
            
            # Verify gradient norms
            root_grad_norm = sum(torch.norm(g).item() for g in local_gradients if g is not None)
            worker_grad_norm = sum(torch.norm(g).item() for g in worker_gradients if g is not None)
            log_timing(f"  Root grad norm: {root_grad_norm:.4f}, Worker grad norm: {worker_grad_norm:.4f}")
        except Exception as e:
            log_timing(f"  ERROR receiving gradients: {e}")
            # Use zeros if reception failed
            worker_gradients = [torch.zeros_like(p) for p in model.parameters()]
            collected_gradients.append(worker_gradients)
        
        # ========== State 3: COMPUTE_THETA ==========
        state_start = wait_for_state(DistributedState.COMPUTE_THETA)
        log_timing(f"Step {step+1}: Computing parameter update", DistributedState.COMPUTE_THETA)
        
        # Average gradients
        averaged_grads = []
        num_nodes = len(collected_gradients)
        
        for param_idx in range(len(param_shapes)):
            grad_sum = None
            for node_grads in collected_gradients:
                if node_grads[param_idx] is not None:
                    if grad_sum is None:
                        grad_sum = node_grads[param_idx].clone()
                    else:
                        grad_sum += node_grads[param_idx]
            
            if grad_sum is not None:
                averaged_grads.append(grad_sum / num_nodes)
            else:
                averaged_grads.append(None)
        
        # Update parameters
        optimizer.zero_grad()
        for param, grad in zip(model.parameters(), averaged_grads):
            if grad is not None:
                param.grad = grad.bfloat16()
        
        torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
        optimizer.step()
        
        # ========== State 4: COMMUNICATE_THETA ==========
        state_start = wait_for_state(DistributedState.COMMUNICATE_THETA)
        log_timing(f"Step {step+1}: Transmitting updated parameters", DistributedState.COMMUNICATE_THETA)
        
        # Send updated parameters
        updated_params = [param.clone().detach() for param in model.parameters()]
        param_frames = tensors_to_frames(updated_params)
        
        phase_start = time.time()
        transmit_frames_properly(cam, param_frames, phase_start)
        
        param_norm = torch.norm(updated_params[0]).item()
        log_timing(f"  Sent parameters with norm: {param_norm:.4f}")
        
        # ========== State 5: COMPUTE_EVAL ==========
        state_start = wait_for_state(DistributedState.COMPUTE_EVAL)
        log_timing(f"Step {step+1}: Evaluating model", DistributedState.COMPUTE_EVAL)
        
        # Evaluate model
        model.eval()
        correct = 0
        total = 0
        
        with torch.no_grad():
            # Just check a few batches for speed
            for i, (inputs, labels) in enumerate(test_loader):
                if i >= 5:  # Only test on 5 batches
                    break
                    
                inputs = inputs.to(device).bfloat16()
                labels = labels.to(device)
                outputs = model(inputs)
                outputs = outputs.float()
                
                _, predicted = torch.max(outputs.data, 1)
                total += labels.size(0)
                correct += (predicted == labels).sum().item()
        
        accuracy = 100 * correct / total if total > 0 else 0
        avg_loss = total_train_loss / (step + 1)
        
        log_timing(f"  Step {step+1}/{args.num_steps}: Accuracy={accuracy:.1f}%, Avg Loss={avg_loss:.4f}")
        
        model.train()
    
    # Final evaluation
    log_timing("\nTraining complete! Running final evaluation...")
    
    model.eval()
    correct = 0
    total = 0
    
    with torch.no_grad():
        for inputs, labels in test_loader:
            inputs = inputs.to(device).bfloat16()
            labels = labels.to(device)
            outputs = model(inputs)
            outputs = outputs.float()
            
            _, predicted = torch.max(outputs.data, 1)
            total += labels.size(0)
            correct += (predicted == labels).sum().item()
    
    final_accuracy = 100 * correct / total
    final_avg_loss = total_train_loss / args.num_steps
    
    log_timing(f"\nFINAL RESULTS:")
    log_timing(f"Test Accuracy: {final_accuracy:.2f}%")
    log_timing(f"Average Train Loss: {final_avg_loss:.4f}")
    
    # Save model
    torch.save(model.state_dict(), 'distributed_model_root.pth')
    log_timing("Model saved to distributed_model_root.pth")
    
    cv2.destroyAllWindows()


if __name__ == "__main__":
    main()
